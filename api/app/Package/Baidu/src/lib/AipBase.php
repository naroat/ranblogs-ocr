...
namespace App\Package\Baidu\src\lib;
...
class AipBase {
    ...
    public function __construct(){
        ...
        $this->appId = trim(env('BAIDU_APP_ID'));
        $this->apiKey = trim(env('BAIDU_API_KEY'));
        $this->secretKey = trim(env('BAIDU_SECRET_KEY'));
        $this->isCloudUser = null;
        $this->client = new AipHttpClient();
        $this->version = '2_2_20';
        $this->proxies = array();
        $this->setAuthFilePath(BASE_PATH . '/runtime/');
    }
    ...
    private function getAuthFilePath(){
        if ($this->authPath != null && is_dir($this->authPath)) {
            $path = $this->authPath . md5($this->apiKey);
        } else {
            $path = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'tmp/' . md5($this->apiKey);
        }
        return $path;
    }
     ...
     * @param $path
     */
    public function setAuthFilePath($path)
    {
        if (!is_dir($path)) {
            throw new \Exception("err：invalid current directory");
        }
        $this->authPath = $path;
    }
